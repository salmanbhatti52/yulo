...
class _MainViewState extends State<MainView> {
                      ...
                      IgnorePointer(
                        ...
                        child: Align(
                          ...
                          child: Text('Tap to type',
                              style: TextStyle(fontFamily: 'Alegreya', package: 'stories_editor', fontWeight: FontWeight.w500, fontSize: 30, color: Colors.white.withOpacity(0.5), shadows: <Shadow>[Shadow(offset: const Offset(1.0, 1.0), blurRadius: 3.0, color: Colors.black45.withOpacity(0.3))])),
                        ),
                      ),
  ...
  void _deleteItemOnCoordinates(EditableItem item, PointerUpEvent details) {
    ...
    } else if (item.type == ItemType.text && item.position.dy >= 0.265 && item.position.dx >= -0.122 && item.position.dx <= 0.122 || item.type == ItemType.gif && item.position.dy >= 0.21 && item.position.dx >= -0.25 && item.position.dx <= 0.25) {
      setState(() {
