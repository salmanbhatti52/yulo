...
import 'package:laravel_chat/src/util/event-formatter.dart';
import 'package:laravel_chat/src/channel/channel.dart';
...
class PusherChannel extends Channel {
  ...
  PusherChannel listenToAll(Function callback) {
    this.subcription.bind_global((String event, dynamic data) {
      ...
      String namespace = this.options['namespace'].replaceAll(new RegExp(r'\.'), '\\');

      String formattedEvent = event.startsWith(namespace) ? event.substring(namespace.length + 1) : '.$event';
