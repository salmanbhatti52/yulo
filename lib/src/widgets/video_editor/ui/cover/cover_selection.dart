...
class _CoverSelectionState extends State<CoverSelection> with AutomaticKeepAliveClientMixin {
  ...
  Stream<List<CoverData>> _generateThumbnails() async* {
    final int duration = widget.controller.isTrimmmed ? (widget.controller.endTrim - widget.controller.startTrim).inMilliseconds : widget.controller.videoDuration.inMilliseconds;
    final double eachPart = duration / widget.nbSelection;
    List<CoverData> _byteList = [];
    for (int i = 0; i < widget.nbSelection; i++) {
      CoverData _bytes = await widget.controller.generateCoverThumbnail(timeMs: (widget.controller.isTrimmmed ? (eachPart * i) + widget.controller.startTrim.inMilliseconds : (eachPart * i)).toInt(), quality: widget.quality);
                              ...
                              return ValueListenableBuilder(
                                  ...
                                  builder: (context, CoverData? selectedCover, __) {
                                    return InkWell(
                                        ...
                                        child: Container(
                                            decoration: BoxDecoration(border: Border.all(color: coverData.sameTime(widget.controller.selectedCoverVal!) ? widget.controller.coverStyle.selectedBorderColor : Colors.transparent, width: widget.controller.coverStyle.selectedBorderWidth)),
                                            child: CropTransform(
                                              transform: transform,
                                              child: Container(
                                                ...
                                                child: Stack(children: [
                                                  Image(image: MemoryImage(coverData.thumbData!), width: _layout.width, height: _layout.height),
                                                  CustomPaint(
                                                    size: _layout,
                                                    painter: CropGridPainter(_rect.value, showGrid: false, style: widget.controller.cropStyle),
                                                  )
                                                ]),
                                              ),
                                            )));
