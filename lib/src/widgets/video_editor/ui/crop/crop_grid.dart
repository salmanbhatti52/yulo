...
enum _CropBoundaries { topLeft, topRight, bottomLeft, bottomRight, inside, topCenter, centerRight, centerLeft, bottomCenter, none }
    ...
    if (_boundary != _CropBoundaries.none) {
      ...
      switch (_boundary) {
        ...
        case _CropBoundaries.topRight:
          _changeRect(
            top: _preferredCropAspectRatio == null ? _rect.value.topRight.dy + delta.dy : (_rect.value.topRight.dy + (delta.dy * _preferredCropAspectRatio!)),
            width: _rect.value.width + delta.dx,
            height: _preferredCropAspectRatio == null ? _rect.value.height - delta.dy : null,
          );
          break;
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder(
      ...
      builder: (_, TransformData transform, __) => Center(
        child: Container(
          constraints: BoxConstraints(maxHeight: ((_controller.rotation == 90 || _controller.rotation == 270)) ? MediaQuery.of(context).size.width - widget.horizontalMargin : Size.infinite.height),
          child: CropTransform(
            transform: transform,
            child: VideoViewer(
