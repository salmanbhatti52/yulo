...
class TrimTimeline extends StatefulWidget {
  ...
  TrimTimeline({Key? key, required this.controller, this.secondGap, this.margin = EdgeInsets.zero}) : super(key: key);
...
class _TrimTimelineState extends State<TrimTimeline> {
  ...
  void initState() {
    ...
    final Duration _duration = widget.controller.maxDuration < widget.controller.videoDuration ? widget.controller.maxDuration : widget.controller.videoDuration;
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          for (int i = 0; i <= (widget.controller.videoDuration.inSeconds / _timeGap).ceil(); i++)
            Text(
              (i * _timeGap <= widget.controller.videoDuration.inSeconds ? i * _timeGap : '').toString(),
            ),
        ]));
  }
}
