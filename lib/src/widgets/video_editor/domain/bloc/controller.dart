...
class VideoEditorController extends ChangeNotifier {
  ...
  Future<void> exportVideo({
    ...
    final String execute = "-y -i $videoPath $filter $trim ${_getPreset(preset)} ${customInstruction ?? ""} $outputPath";
    ...
    await FFmpegKit.executeAsync(
      ...
      (session) async {
        ...
        print("FFmpeg process exited with state $state and return code $code.${(failStackTrace == null) ? "" : "\\n" + failStackTrace}");
        final logs = await session.getLogs();
        print("ffmpegLogs123 $logs");
